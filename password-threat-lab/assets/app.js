(function () {
  ...
  const FALLBACK_CONFIG = Object.freeze({
    ...
    limits: {
      maximum_password_characters: 256,
      maximum_custom_guesses_per_second: 1e18
    }
  });
  ...
  const dom = {
    ...
    profile: document.getElementById("attackProfile"),
    profileDescription: document.getElementById("profileDescription"),
    customGroup: document.getElementById("customRateGroup"),
    customRate: document.getElementById("customRate"),
    analyzeStatus: document.getElementById("analysisStatus"),
    emptyState: document.getElementById("emptyState"),
    resultState: document.getElementById("resultState"),
    strengthLabel: document.getElementById("strengthLabel"),
    strengthMeter: document.getElementById("strengthMeter"),
    method: document.getElementById("patternMethod"),
    explanation: document.getElementById("patternExplanation"),
    patternBits: document.getElementById("patternBits"),
    patternGuesses: document.getElementById("patternGuesses"),
    patternTime: document.getElementById("patternTime"),
    randomBits: document.getElementById("randomBits"),
    randomGuesses: document.getElementById("randomGuesses"),
    randomTime: document.getElementById("randomTime"),
    groverBits: document.getElementById("groverBits"),
    groverQueries: document.getElementById("groverQueries"),
    selectedRate: document.getElementById("selectedRate"),
    signals: document.getElementById("patternSignals"),
  ...
  function populateProfiles() {
    ...
    customOption.textContent = "Custom illustrative rate";
    dom.profile.append(customOption);

    dom.profile.value = profilesById.has(config.default_profile_id)
  ...
  async function loadConfig() {
    try {
      const response = await fetch("data/attack-profiles.json", {
        cache: "no-store",
  ...
  function validateConfig(candidate) {
    if (!candidate || !Array.isArray(candidate.profiles) || candidate.profiles.length === 0) {
    ...
    for (const profile of candidate.profiles) {
      if (
        ...
        !Number.isFinite(profile.guesses_per_second) ||
        profile.guesses_per_second <= 0
      ) {
  ...
  function selectedScenario() {
    if (dom.profile.value === "custom") {
      ...
      const rate = Number.isFinite(parsed) && parsed > 0 ? Math.min(parsed, maximum) : 1;
      return {
        id: "custom",
        label: "Custom illustrative rate",
  ...
  function updateProfileDescription() {
    const isCustom = dom.profile.value === "custom";
    dom.customGroup.hidden = !isCustom;
    const scenario = selectedScenario();
    dom.profileDescription.textContent = scenario.description;
  }

  function clearList(element, items, emptyText) {
  ...
  function render() {
    ...
    dom.count.textContent = `${length} / ${dom.password.maxLength}`;
    ...
    clearList(dom.signals, result.signals, "No bundled shortcut pattern was detected.");
    clearList(dom.suggestions, result.suggestions, "Use a unique generated password and phishing-resistant MFA.");
