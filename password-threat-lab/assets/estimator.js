(function (root, factory) {
  ...
  function safeLog2(value) {
    return Math.max(0, Math.log2(Math.max(1, value)));
  }
  ...
  function caseVariantMultiplier(value) {
    ...
    if (letters === 0 || value === value.toLowerCase()) {
      return 1;
    }
    if (value === value.toUpperCase() || /^[A-Z][a-z]+$/.test(value)) {
      return 2;
    }
  ...
  function candidate(guessBits, method, explanation, signal) {
    return {
      guessBits: Math.max(1, guessBits),
      method,
      explanation,
      signal
    };
  ...
  function commonPasswordCandidate(password) {
    ...
    const directRank = COMMON_INDEX.get(lowered);
    const normalizedRank = COMMON_INDEX.get(normalized);
    const rank = directRank || normalizedRank;
    if (!rank) return null;
  ...
  function repeatCandidate(password) {
    ...
    for (let blockLength = 1; blockLength <= Math.floor(characters.length / 2); blockLength += 1) {
      ...
      if (block.repeat(repeats) === password) {
        const alphabet = inferredAlphabetSize(block);
        const blockBits = blockLength * Math.log2(alphabet);
        return candidate(
          blockBits + Math.log2(repeats) + 2,
          "repeated block",
  ...
  function phraseCandidate(password) {
    ...
    const reconstructedLength = tokens.join("").length + separators.join("").length;
    if (reconstructedLength !== password.length) return null;
    ...
    return candidate(
      dictionaryBits + separatorBits + caseBits - 1,
      "word-based phrase",
  ...
  function randomSpaceCandidate(password) {
    ...
    const averageGuessBits = Math.max(1, length * Math.log2(alphabet) - 1);
    return candidate(
      averageGuessBits,
  ...
  function buildSuggestions(selected, bits) {
    const suggestions = [];
    const predictableMethods = new Set([
      "common-password match",
      "word with predictable suffix",
      "keyboard or character sequence",
      "repeated character",
    ...
    if (predictableMethods.has(selected.method)) {
      suggestions.push("Replace words, dates, sequences, and substitutions with genuinely random choices.");
    }
    if (bits < 64) {
      suggestions.push("Use a password manager to generate at least 16 random characters, or use 4–6 randomly selected words.");
    }
  ...
  function estimatePassword(password) {
    ...
    const builders = [
      commonPasswordCandidate,
      wordPlusSuffixCandidate,
      sequenceCandidate,
      repeatCandidate,
      dateCandidate,
      phraseCandidate
    ...
    const signals = candidates
      .filter((item) => item.method !== "character-space estimate")
      .map((item) => item.signal);

    return {
      length: codePointLength(password),
      pattern: selected,
      random,
      candidates,
      score,
      ...
      signals,
      grover: {
        queryBits: random.guessBits / 2,
        basis: "random-space square-root query proxy"
      }
    };
  }

  function log10CountFromBits(bits) {
    return bits * LOG10_2;
  }
  ...
  function formatCountFromBits(bits) {
    ...
    if (log10Value < 15) {
      const value = Math.max(1, Math.round(10 ** log10Value));
  ...
  function formatScaled(log10Value, unit) {
    ...
    let text;
    if (value < 10) text = value.toFixed(2);
    else if (value < 100) text = value.toFixed(1);
    else text = value.toFixed(0);
    const plural = Math.abs(value - 1) < 0.005 ? "" : "s";
  ...
  function formatDuration(guessBits, guessesPerSecond) {
    if (!Number.isFinite(guessesPerSecond) || guessesPerSecond <= 0) {
      throw new Error("guessesPerSecond must be a positive finite number");
    }

    const log10Seconds = log10CountFromBits(guessBits) - Math.log10(guessesPerSecond);
    if (log10Seconds < -6) return "under 1 microsecond";
    if (log10Seconds < -3) return formatScaled(log10Seconds + 6, "microsecond");
    if (log10Seconds < 0) return formatScaled(log10Seconds + 3, "millisecond");
    if (log10Seconds < Math.log10(60)) return formatScaled(log10Seconds, "second");
    if (log10Seconds < Math.log10(3600)) return formatScaled(log10Seconds - Math.log10(60), "minute");
    if (log10Seconds < Math.log10(86_400)) return formatScaled(log10Seconds - Math.log10(3600), "hour");
    if (log10Seconds < Math.log10(SECONDS_PER_YEAR)) {
      return formatScaled(log10Seconds - Math.log10(86_400), "day");
    }

    const log10Years = log10Seconds - Math.log10(SECONDS_PER_YEAR);
    if (log10Years < 6) return formatScaled(log10Years, "year");
    const exponent = Math.floor(log10Years);
    const mantissa = 10 ** (log10Years - exponent);
    return `≈ ${mantissa.toFixed(2)} × 10^${exponent} years`;
  }
  ...
  function formatRate(rate) {
    ...
    for (const [divisor, label] of units) {
      if (rate >= divisor) {
        return `${(rate / divisor).toLocaleString("en-US", { maximumFractionDigits: 2 })} ${label}/s`;
      }
    }
